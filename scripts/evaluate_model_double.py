...
def create_heatmap(heatmap_data: pd.DataFrame, output_dir: Path) -> None:
    ...
    plt.title('Test MCC Scores for Bacterial Pair Models', fontsize=16, pad=20)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate double bacteria AMR prediction models and create heatmap"
    )
    parser.add_argument(
        "results_dir",
        type=str,
        help="Path to the directory containing the bacterial pair results"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
